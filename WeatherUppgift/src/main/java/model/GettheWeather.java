...
public class GettheWeather {
	...
	public static void getWeather(weatherBean wBean) throws IOException {
		...
		//api.openweathermap.org/data/2.5/weather?q={city name}&appid={f5d72ad7d807f64c26c4f56afbbff9a5}
		
		
		String URLtoSend = "http://api.openweathermap.org/data/2.5/weather?q=" + wBean.getCityStr() + ","
				+ wBean.getCountryStr() + "&APPID=f5d72ad7d807f64c26c4f56afbbff9a5&mode=xml";
		...
		NodeList tList = doc.getElementsByTagName("lastupdate");
		...
		for (int temp = 0; temp < tList.getLength(); temp++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				String XMLtime = tElement.getAttribute("value");
				
				System.out.println("Current time is: " + XMLtime);
				
				wBean.setCurrentTime(XMLtime);
			}
		}
		NodeList hList = doc.getElementsByTagName("humidity");
		
		for (int temp = 0; temp <hList.getLength(); temp++) {
			Node node = hList.item(temp);
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				Element hElement = (Element) node;
				
				String XMLhumidity = hElement.getAttribute("value");
				
				System.out.println("Humidity level is: " + XMLhumidity + "%");
				
				wBean.setCurrentHumidity(XMLhumidity);
			}
		}
